...
qx.Class.define("qooxdo_proj.components.Tabs.StudentInfoTable", {
    ...
    members: {
        ...
        _showEditDialog: function (student) {
            ...
            const yearLevelItems = [1, 2, 3, 4].map((n, i) => new qx.ui.form.ListItem(["1st", "2nd", "3rd", "4th"][i] + " Year", n));
            ...
            if (student.yearLevel != null && student.yearLevel >= 1 && student.yearLevel <= 4) {
                ...
                if (item)
                    yearLevelField.setSelection([item]);
            }
            ...
            qx.event.Timer.once(() => {
                ...
                if (student.program) {
                    const items = programField.getChildren();
                    for (let i = 0; i < items.length; i++) {
                        if (items[i].getLabel() === student.program) {
                            programField.setSelection([items[i]]);
                            break;
                        }
                    }
                }
                if (student.yearLevel != null && student.yearLevel >= 1 && student.yearLevel <= 4) {
                    const items = yearLevelField.getChildren();
                    for (let i = 0; i < items.length; i++) {
                        if (items[i].getModel() === student.yearLevel) {
                            yearLevelField.setSelection([items[i]]);
                            break;
                        }
                    }
                }
            ...
            saveButton.addListener("execute", () => {
                ...
                if (dobValue)
                    dateOfBirthValue = dobValue instanceof Date ? dobValue.toISOString().split("T")[0] : dobValue;
                let genderValue = "";
                const gs = genderField.getSelection();
                if (gs && gs.length > 0)
                    genderValue = gs[0].getLabel();
                this._saveStudent(student.id, {
                    ...
                    program: (programField.getSelection() && programField.getSelection().length > 0) ? programField.getSelection()[0].getLabel() : "",
                    yearLevel: (yearLevelField.getSelection() && yearLevelField.getSelection().length > 0) ? yearLevelField.getSelection()[0].getModel() : null,
            ...
            this._editWindow.addListenerOnce("appear", () => {
                ...
                if (s.studentId)
                    f.studentId.setValue(s.studentId);
                if (s.firstName)
                    f.firstName.setValue(s.firstName);
                if (s.lastName)
                    f.lastName.setValue(s.lastName);
                if (s.dateOfBirth) {
                    try {
                ...
                if (s.gender)
                    f.gender.setValue(s.gender);
                if (s.address)
                    f.address.setValue(s.address);
                if (s.email)
                    f.email.setValue(s.email);
                if (s.personalPhone)
                    f.personalPhone.setValue(s.personalPhone);
                if (s.emergencyContact)
                    f.emergencyContact.setValue(s.emergencyContact);
                if (s.emergencyContactPhone)
                    f.emergencyContactPhone.setValue(s.emergencyContactPhone);
                if (s.relationship)
                    f.relationship.setValue(s.relationship);
                if (s.program)
                    f.program.setValue(s.program);
                if (s.yearLevel != null && s.yearLevel >= 1 && s.yearLevel <= 4) {
                    const items = f.yearLevel.getChildren();
                    for (let i = 0; i < items.length; i++) {
                        if (items[i].getModel() === s.yearLevel) {
                            f.yearLevel.setSelection([items[i]]);
                            break;
                        }
                    }
                }
                if (s.gradeSchool)
                    f.gradeSchool.setValue(s.gradeSchool);
                if (s.highSchool)
                    f.highSchool.setValue(s.highSchool);
                if (s.college)
                    f.college.setValue(s.college);
            }, this);
        ...
        _saveStudent: function (studentId, studentData) {
            qooxdo_proj.utils.GraphQLClient.updateStudent(studentId, studentData)
                ...
                .catch((error) => { console.error("Failed to update student:", error); alert("Failed to update student: " + error.message); });
        },
        ...
        _deleteStudent: function (studentId) {
            qooxdo_proj.utils.GraphQLClient.deleteStudent(studentId)
                .then(() => this.loadStudents())
                .catch((error) => { console.error("Failed to delete student:", error); alert("Failed to delete student: " + error.message); });
        },
        ...
        loadStudents: function () {
            qooxdo_proj.utils.GraphQLClient.getStudents()
                ...
                .catch((error) => console.error("Failed to load students from API:", error));
        }
    }
});
